...
export function validateAction(action: AgentAction): SafetyResult {
  ...
  if (
    ...
    action.action === 'memory_update_core' ||
    action.action === 'soul_update'
  ) {
  ...
  if (action.action === 'type' || action.action === 'fill') {
    for (const pat of SENSITIVE_PATTERNS) {
  ...
  if (action.action === 'open_url' || action.action === 'navigate_page') {
    const candidate = (action.target || action.value || '').trim().toLowerCase();
    const navCmd = candidate.replace(/^action:\s*/i, '');
    const isNavKeyword =
      navCmd === 'back' ||
      navCmd === 'forward' ||
      navCmd === 'reload' ||
