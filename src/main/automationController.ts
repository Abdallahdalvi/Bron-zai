...
export class RendererAutomationController implements AgentAutomationController {
  ...
  async getScreenshot(): Promise<string> {
    return await this.invoke<string>('getScreenshot', { tabId: this.agentTabId });
  }

  async getDom(selector?: string): Promise<string> {
    return await this.invoke<string>('getDom', { tabId: this.agentTabId, selector: selector || '' });
  }

  async searchDom(query: string, limit = 25): Promise<string> {
    return await this.invoke<string>('searchDom', { tabId: this.agentTabId, query, limit });
  }

  async evaluateScript(script: string): Promise<string> {
    return await this.invoke<string>('evaluateScript', { tabId: this.agentTabId, script });
  }

  async getConsoleLogs(options?: { clear?: boolean; level?: string; limit?: number; search?: string }): Promise<string> {
    return await this.invoke<string>('getConsoleLogs', { tabId: this.agentTabId, ...(options || {}) });
  }
  ...
  async goBack(): Promise<void> {
    await this.invoke<boolean>('goBack', { tabId: this.agentTabId });
  }

  async goForward(): Promise<void> {
    await this.invoke<boolean>('goForward', { tabId: this.agentTabId });
  }

  async refresh(): Promise<void> {
    await this.invoke<boolean>('refresh', { tabId: this.agentTabId });
  }
