...
const pattern = /<div className="flex items-center gap-2">\\s*<span className="text-\\[10px\\] text-slate-400 font-bold uppercase tracking-tighter mr-1">Export Result<\\/span>.*?<RotateCcw className="w-3.5 h-3.5" \\/>\\s*<\\/button>\\s*<\\/div>/s;

let newContent = content.replace(pattern, newButtons);

if (newContent === content) {
    const fallbackPattern = /<div className="flex items-center gap-2">.*?Export Result.*?<RotateCcw className="w-3.5 h-3.5" \\/>\\s*<\\/button>\\s*<\\/div>/s;
    newContent = content.replace(fallbackPattern, newButtons);
}

fs.writeFileSync(filePath, newContent);
console.log('Successfully patched AgentSidebar.tsx');
